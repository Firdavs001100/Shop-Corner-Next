...
const NewProductCard = (props: NewProductCardProps) => {
	...
	const handleAddToCart = (e: React.MouseEvent, product: any) => {
		...
		addToCart({
			...
			price: finalPrice,
			image: `${process.env.REACT_APP_API_URL}/${product.productImages[0]}`,
			quantity: 1,
		});
	};
	...
	return (
		<Box className="new-product-card">
			<div className="image-box" onClick={() => pushDetailHandler(product._id)}>
				...
				{product.productImages?.[1] && (
					<img
						src={`${process.env.REACT_APP_API_URL}/${product.productImages[1]}`}
				...
				{/* <div className="like-box top-right">
					<IconButton
						onClick={(e) => {
							e.stopPropagation();
					...
					>
						{product?.meLiked?.[0]?.myFavorite ? <FavoriteIcon style={{ color: 'red' }} /> : <FavoriteIcon />}
					</IconButton>
