...
const MyArticles = () => {
	...
	const buildQuery = (): BoardArticlesInquiry => ({
		page,
		limit: PAGE_SIZE,
		search: {
			memberId: user?._id,
	...
	const likeHandler = async (e: React.MouseEvent, articleId: string) => {
		...
		try {
			...
			await refetch({ input: buildQuery() });
		} catch (err: any) {
	...
	const deleteHandler = async (e: React.MouseEvent, articleId: string) => {
		...
		try {
			await deleteArticle({ variables: { input: { _id: articleId, articleStatus: 'DELETE' } } });
	...
	return (
		<div className="mp-my-articles">
			...
			{loading ? (
				...
				<div className="mp-my-articles__list">
					{articles.map((article) => (
						<div key={article._id} className="mp-my-articles__card-wrap">
							{/* ArticleCard handles display, like, navigation */}
							<ArticleCard article={article} onLike={likeHandler} />
